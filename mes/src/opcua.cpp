...
// Recebe como parâmetros de entrada o cliente e uma string com o nome da
// variável a ler
int OPCUA_readInt(UA_Client *client, const char *var) {

  char str[100] =
      "|var|CODESYS Control Win V3 x64.Application.MAIN."; // Mapeamento da
                                                           // variável "var"
  strcat_s(str, var);

  UA_Variant value;        // declaração da
  UA_Variant_init(&value); // variável value
  while (UA_STATUSCODE_GOOD !=
         UA_Client_readValueAttribute(client, UA_NODEID_STRING(4, str), &value))
    ; // Guarda em "value" o valor da variável "var"

  return *(UA_Int16 *)value.data;
}

// Escreve uma variável do tipo inteiro
// Recebe como parâmetros de entrada o cliente, uma string com o nome da
// variável e o valor a escrever
void OPCUA_writeInt(UA_Client *client, const char *var, int val) {
  ...
  while (UA_STATUSCODE_GOOD != UA_Client_writeValueAttribute(
                                   client, UA_NODEID_STRING(4, str), &newval))
    ;
}
...
bool OPCUA_readBool(UA_Client *client, const char *var) {
  ...
  UA_Client_readValueAttribute(
      client, UA_NODEID_STRING(4, str),
      &value); // Guarda em "value" o valor da variável "var"
